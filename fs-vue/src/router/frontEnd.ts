	...
	chil.forEach((route: any) => {
		if (route.meta.roles) {
			route.meta.roles.forEach((metaRoles: any) => {
				userStore.roleList.forEach((roles: any) => {
...
export function setCacheTagsViewRoutes() {
	...
	const storesTagsView = useTagsViewRoutes(store);
	let rolesRoutes = setFilterHasRolesMenu(dynamicRoutes, userStore.roleList);
...
export function setFilterMenuAndCacheTagsViewRoutes() {
	...
	storesRoutesList.setRoutesList(setFilterHasRolesMenu(dynamicRoutes[0].children, userStore.roleList));
